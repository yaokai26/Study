...
ArrayList是基于数组实现的，而LinkedList是基于链表实现的。ArrayList在存储的时候不仅仅是存储了数据，还给每一个数据存储了一个引用即数组下标，而在LinkedList中是没有下标的。因此，ArrayList可以通过数组下标迅速地找到要查询的数据，LinkedList查询需要重新遍历一次链表，在遍历中逐个查询，知道找到index为止。简单来说，ArrayList可以直接通过数组下标得到元素，而LinkedList则需要根据所给的下标从头部或尾部开始往下标的位置依次获得下一个元素或上一个元素。至于增删，由于ArrayList是以数组下标(引用)＋数据存储的。例如，在一个长度为10的数组中，在数组A[3]位置插入一个数据时，数组会先扩容至长度11，然后A[4]-A[9]的值会推一位，重新赋予数组下标，这时候添增的效率就好变得非常慢。而LinkedList中的添加，就不用考虑引用，直接在链尾添加一个数据就行了。
